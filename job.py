...
class PSDDealer:
    ...
    def process_psd(self, file_path):
        ...
        png_dir_path = "%s/__%s__.gen" % (
            self.conf["psd_handler"]["src_dir_psd"], os.path.basename(file_path)[:-4])
...
class TextureDealer:
    ...
    def process_texture(self):
        ...
        images = [(os.path.basename(image_dir)[:-4], Image.open(image_dir))
                  for image_dir in image_dirs]

        # sort list by width * height
        # from small to big
        images = sorted(
            images, key=lambda (name, image): image.size[0] * image.size[1])
        ...
        pack_size = (self.conf["psd_handler"]["texture_size"][
                     "width"], self.conf["psd_handler"]["texture_size"]["height"])
        tree = Node(pack_size)
        ...
        image.save("%s/textureAtlas.png" %
                   (self.conf["psd_handler"]["trg_dir"]), "PNG")
        open("%s/atlas_data.json" %
             (self.conf["psd_handler"]["trg_dir"]), "w").write(json.dumps(data, indent=4))


if __name__ == "__main__":
    """ TODO: delete these code block """
